...
def run_task_in_process(week, date, task, member_id, log_dir, task_id, output_dir, temperature=0):
    run_task(week, date, task, member_id, log_dir, task_id, output_dir=output_dir, temperature=temperature)
...
class Member:
    def __init__(self, member_id, week, date, member_config_dir, schedule_dir, log_dir, member_id_list, id_role_map, attack_id, attacker):
        ...
        if attacker:
            self.schedule_file = os.path.join(config.attack_schedule_dir, f'{member_id}_week_{self.week}_{self.date}_attack.json')
        ...
        if self.attacker:
            ...
            with open(os.path.join(config.attack_dir, f"{attack_id}.json"), "r") as fattack:
    ...
    def reply_email(self, current_time):
        ...
        if reply_email:
            ...
            # log the email
            self.email_logging(datetime.now(), current_time, email_info, attack_activity=False)
    ...
    def update_schedule(self, incom_email_data, reply_email_data, current_time):
        ...
        updated_schedule = update_daily_schedule_attack(self.schedule, self.member_profile, incom_email_data, reply_email_data, current_time, self.id_role_map, self.attacker, attack_info, self.previous_summary)
    ...
    def execute_task(self, activity, current_time, attack_activity):
        ...
        if "break" in activity:
            ...
            if "@" in activity:
                t = Thread(target=self.send_email, args=(activity, current_time, attack_activity,))
                t.daemon = True
                t.start()
                ...
                process = Process(
                    ...
                    args=(self.week, self.date, activity, self.id, self.logging_dir, self.execution_task_id, self.temp_dir, task_temperature)
                    )
                process.start()
                # process.join()
        ...
        self.schedule_logging(datetime.now(), current_time, activity, attack_activity)
        self.execution_task_id += 1
    ...
    def logon_logging(self, real_time, sim_time, status, attack_activity):
        ...
        with LOGON_LOCK:
            with open(log_file, mode="a", newline="") as csvfile:
                ...
                writer.writerow([write_id, real_timestamp, sim_timestamp, self.name, self.container_id, status])
    
    def schedule_logging(self, real_time, current_time, activity, attack_activity=False):
        ...
        with SCHEDULE_LOCK:
            with open(log_file, mode="a", newline="") as csvfile:
                ...
                writer.writerow([write_id, self.execution_task_id, real_timestamp, sim_timestamp, self.name, self.container_id, activity])
    ...
    def email_logging(self, real_time, sim_time, email_info, attack_activity):
        ...
        with EMAIL_LOCK:
            with open(log_file, mode="a", newline="") as csvfile:
                ...
                writer.writerow([write_id, real_timestamp, sim_timestamp, self.name, email_to, email_cc, email_info["subject"], email_info["content"].replace("\\", "\\\\").replace("\n", "\\n").replace("\r", "\\r")])

if __name__ == "__main__":

    parser = argparse.ArgumentParser(description="Attack execution for existing schedules and members.")
    ...
    daemon_log_path = os.path.join(daemon_log_dir, f"daemon_week_{attack_week}_{attack_date}_attack.log")
    ...
    print(f"[INFO][Attack] Start time for week {attack_week} - {attack_date} is {start_time.strftime('%H:%M:%S')}.")
    ...
    print(f"[INFO][Attack] All members have completed their tasks for week {attack_week} - date {attack_date}.")

    # remove the attack schedule file
    attack_schedule_file = os.path.join(config.attack_schedule_dir, f'{attacker_id}_week_{attack_week}_{attack_date}_attack.json')
