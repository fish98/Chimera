...
def update_daily_schedule_with_gpt(previous_schedule, member_profile, incom_email_data, reply_email_data, current_time, id_role_map, previous_summary=None):
...
def update_daily_schedule_with_gpt_attack(previous_schedule, member_profile, incom_email_data, reply_email_data, current_time, id_role_map, attack_info, previous_summary=None):
...
def update_daily_schedule(previous_schedule, member_profile, incom_email_data, reply_email_data, current_time, id_role_map, previous_summary=None):

    for attempt in range(config.max_attempt):

        output_schedule = update_daily_schedule_with_gpt(previous_schedule, member_profile, incom_email_data, reply_email_data, current_time, id_role_map, previous_summary)
        # # Parse the response to extract the updated schedule
        if "```json" in output_schedule:
            output_schedule = output_schedule.replace("```json", "").replace("```", "").strip()
...
def update_daily_schedule_attack(previous_schedule, member_profile, incom_email_data, reply_email_data, current_time, id_role_map, attacker, attack_info, previous_summary=None):

    for attempt in range(config.max_attempt):

        if attacker:
            output_schedule = update_daily_schedule_with_gpt_attack(previous_schedule, member_profile, incom_email_data, reply_email_data, current_time, id_role_map, attack_info, previous_summary)
        else:
            output_schedule = update_daily_schedule_with_gpt(previous_schedule, member_profile, incom_email_data, reply_email_data, current_time, id_role_map, previous_summary)
