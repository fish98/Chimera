...
def get_member_profile(role, id, ip, existing_profiles):
    ...
    for attempt in range(config.max_attempt):
        ...
        user_prompt = f"""Now, please generate one profile config for: one {role}. Try using diversed names and personalities for each profile. Existing profiles are: {existing_profiles}.\n\n"""
        ...
        try:
            ...
            if attempt == config.max_attempt - 1:
                print(f"[Error] Max attempts reached. The profile generation for {id} failed.")
...
if __name__ == "__main__":
    ...
    for role in tqdm.tqdm(all_roles, desc="Generating profiles... "):
        ...
        for i in range(role_number):
            ...
            id = f"{role['abbr']}-{i+1}"
            member_profile = get_member_profile(role['role_name'], id, ip_start, existing_profiles)
