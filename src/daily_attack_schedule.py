...
def attack_daily_schedule_with_gpt(initial_schedule, member_profile, attack_info, id_role_map):
...
def update_daily_schedule_with_attack(week, date, member_id, attack_id, id_role_map):
    ...
    if os.path.exists(f"{config.init_schedule_dir}/week_{week}/{member_id}_week_{week}_{date}.json"):
        # if not exist then choose the first schedule in the week as default
        with open(f"{config.init_schedule_dir}/week_{week}/{member_id}_week_{week}_{date}.json", 'r') as f:
            initial_schedule = json.load(f)
    else:
        with open(f"{config.init_schedule_dir}/week_{week}/{member_id}_week_{week}_Monday.json", 'r') as f:
            initial_schedule = json.load(f)
    ...
    for attempt in range(config.max_attempt):
        ...
        output_schedule = attack_daily_schedule_with_gpt(initial_schedule, member_profile, attack_info, id_role_map)
    ...
    with open(f"{config.attack_schedule_dir}/{member_id}_week_{week}_{date}_attack.json", 'w') as f:
